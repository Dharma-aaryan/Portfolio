...
const ImpactHighlights = () => {
  const highlights: ImpactHighlight[] = [
    {
      icon: (
        <Briefcase className="text-[var(--portfolio-text-primary)]" size={20} />
      ),
      number: "8+",
    ...
    {
      icon: (
        <Award className="text-[var(--portfolio-text-primary)]" size={20} />
      ),
      number: "10+",
    ...
    {
      icon: (
        <BookOpen className="text-[var(--portfolio-text-primary)]" size={20} />
      ),
      number: "1",
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
      {highlights.map((highlight, index) => (
        <div
          key={index}
          className="bg-[var(--portfolio-primary)]/50 p-6 rounded-2xl border border-[var(--portfolio-secondary)] hover-lift text-center"
        >
          ...
          <div className="text-sm text-[var(--portfolio-text-secondary)]">
            {highlight.label}
          </div>
        </div>
      ))}
...
const ExperienceCard = ({ item }: { item: ExperienceItem }) => (
  <div className="bg-[var(--portfolio-primary)]/50 p-6 rounded-2xl border border-[var(--portfolio-secondary)] hover-lift shadow-lg hover:shadow-xl transition-all duration-300">
    <div className="flex items-start space-x-4">
      {item.icon && (
        <div
          className="flex-shrink-0 w-12 h-12 rounded-xl overflow-hidden p-2 mt-1 shadow-sm relative"
          style={{
            backgroundColor: "#fff",
          }}
        >
      ...
      <div className="flex-1 min-w-0">
        <h4 className="text-lg font-bold text-[var(--portfolio-accent)] mb-2 leading-tight">
          {item.title}
        </h4>
          ...
          <ul className="text-sm text-[var(--portfolio-text-secondary)] leading-relaxed space-y-1 mt-3">
            {item.bullets.map((bullet, index) => (
              <li key={index} className="flex items-start">
                <span className="text-[var(--portfolio-accent)] mr-2 mt-1">
                  •
                </span>
                <span>{bullet}</span>
              </li>
...
const TabCard = ({ item }: { item: ExperienceItem }) => (
  <div className="bg-[var(--portfolio-primary)]/50 p-8 rounded-2xl border border-[var(--portfolio-secondary)] hover-lift hover:shadow-[0_20px_60px_rgba(0,191,255,0.25)] transition-all duration-300 w-full max-w-none cursor-pointer">
    <div className="flex items-start space-x-6">
      {item.icon && (
        <div
          className="flex-shrink-0 w-14 h-14 rounded-xl overflow-hidden p-2 mt-1 shadow-sm relative"
          style={{
      ...
      <div className="flex-1 min-w-0">
        <h4 className="text-xl font-bold text-[hsl(195,100%,50%)] mb-3 leading-tight">
          {item.title}
        </h4>
        <p className="text-base mb-1">
          <span className="text-[var(--portfolio-accent)] font-medium">
            {item.organization}
          </span>{" "}
          •
          <span className="text-[var(--portfolio-text-primary)] font-medium">
            {" "}
            {item.period}
          </span>
          {item.location && (
            <span className="text-[var(--portfolio-text-primary)]">
              {" "}
              • {item.location}
            </span>
          )}
        ...
        {item.bullets && item.bullets.length > 0 ? (
          <ul className="text-base text-[var(--portfolio-text-secondary)] leading-relaxed space-y-2 mt-4">
            {item.bullets.map((bullet, index) => (
              <li key={index} className="flex items-start">
                <span className="text-[var(--portfolio-accent)] mr-3 mt-1">
                  •
                </span>
                <span>{bullet}</span>
              </li>
...
export default function AboutSection() {
  const [activeTab, setActiveTab] = useState<
    "professional" | "education" | "leadership" | "research"
  >("professional");
  ...
  const research: ExperienceItem[] = [
    {
      title:
        "Enhancing Healthcare Outcomes through Business Intelligence and Real-Time Analytics",
      organization: "IJECBS",
      period: "December 2022",
      location: "Research Publication",
      description:
        "This research explores the integration of business intelligence systems and real-time analytics to improve healthcare delivery, operational efficiency, and patient outcomes in hospital environments.",
      type: "publication",
      startDate: "2022-07",
      bullets: [
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-[var(--portfolio-text-secondary)] max-w-3xl mx-auto mb-12">
            ...
            boundaries of modern technologies
          </p>
        </div>
